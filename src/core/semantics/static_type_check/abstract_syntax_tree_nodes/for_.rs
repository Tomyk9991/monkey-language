...
impl StaticTypeCheck for For {
    fn static_type_check(&self, type_context: &mut StaticTypeContext) -> Result<(), StaticTypeCheckError> {
        ...
        let condition_type = self.condition.get_type(type_context).ok_or(
            StaticTypeCheckError::InferredError(InferTypeError::NoTypePresent(
        ...
        if self.update.define {
            return Err(StaticTypeCheckError::InferredError(InferTypeError::DefineNotAllowed(self.update.clone(), self.file_position.clone())));
        }
