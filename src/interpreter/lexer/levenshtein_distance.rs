...
fn summarize_method_call_statement(values: &mut Vec<String>) {
    let mut new_vec = vec![];
...
/// puts together segments, with the following pattern
fn summarize_quote_statements(values: &mut Vec<String>) {
    let mut new_vec = vec![];
